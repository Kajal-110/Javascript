...
addNewRow = () => {
  ...
  call1.innerHTML = `${sNo++} `
  call2.innerHTML = '<input type="text" placeholder="Name" class=" name" id="name" name="name" / > '
...
function validateData(){
  ...
  for(i=0;i<name.length;i++){
    ...
    if (marksVal !== "") {
      if (! (/^\d*(?:\.\d{0,2})?$/.test(marksVal))) {
          alert("Please enter number only that is between 1 - 100");
          marksVal.focus();
          return false;
      }
    }
    if(!regName.test(nameVal) || !regName.test(subjectVal)){
      alert("name and subject contains latters only")
      return false
    }
  }
   if(isFormValid){
